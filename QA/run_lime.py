...
def run_lime(args, tokenizer, model, inputs, feature):    
...
def predict_and_run_lime(args, batch, model, tokenizer, batch_features, batch_examples):
    model.eval()
    ...
    with torch.no_grad():
        importances = run_lime(args, tokenizer, model, inputs, batch_features[0])

    return batch_predictions, batch_prelim_results, importances

def lime_interp(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        batch_predictions, batch_prelim_results, batch_importances = predict_and_run_lime(
            args,
            batch,
            model,
            tokenizer,
        ...
        dump_lime_info(args, batch_examples, batch_features, tokenizer, batch_predictions, batch_prelim_results, batch_importances)
...
def dump_lime_info(args, examples, features, tokenizer, predictions, prelim_results, attributions):
...
def main():
    ...
    if args.do_vis:
        ...
        # Evaluate
        result = lime_interp(args, model, tokenizer, prefix="")
