...
from int_grad.ig_models import IGRobertaLayer
...
class ProbeRobertaAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = ProbeRobertaSelfAttention(config)
...
class ProbeRobertaLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = ProbeRobertaAttention(config)
...
class ProbeRobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = ProbeRobertaEncoder(config)
...
class ProbeRobertaForQuestionAnswering(RobertaPreTrainedModel):
