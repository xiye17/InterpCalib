...
from common.tokenizer_tools import merge_tokens_into_words
...
def visualize_token_attributions(args, tokenizer, interp_info):
    ...
    feature = interp_info['example']

    # prefix    
    prefix = join(args.visual_dir, f'{feature.idx}-{feature.pair_id}')
...
def visualize_attn_attributions(args, tokenizer, interp_info, do_layer=False, do_head=False):
    assert args.visual_dir is not None
    feature = interp_info['example']
    ...
    if 'active_layers' in interp_info:
    ...
    visualize_connection(prefix + '-aggregated.jpg', words, aggregated_attribution, interp_info)
